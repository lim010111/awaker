...
**2026-07-20 (agent, 실기기 파리티 판정)**: 통합 APK(ddb9a2f) 설치 후 adb
인터리브 시나리오 2회 — **전 세션 일치, 합격**.

- **시나리오 A** (직행 전환 7회·앱 3개 인터리브, 룰 전이 없음): Chrome·YouTube·
  sbrowser 3세션 전부 기기 0전이 ↔ replay 0전이 일치. 단, 전이가 없어 판별력
  없는 공허한 통과 — 플링 스팬이 enter 문턱(≥8회+스팬 30s)에 미달했음을
  확인하고 B로 재설계.
- **시나리오 B** (판별력 확보): sbrowser Start → Chrome Start(플링 14회@3s →
  enter) → YouTube Start(**마지막 Started 미끼**) → sbrowser **Resume** 지속
  플링(enter → dwell 90s → 시트 shown) → Chrome 전환(경계 Dismiss) → HOME.
  결과: **Chrome enter Δ0ms · sbrowser enter Δ0ms · YouTube 0↔0 — 3세션 일치**.
  기기가 경계에서 무음 리셋한 것(전이 라인 enter 1건뿐)을 replay가 foreground
  라인으로 동일하게 재현 — ADR-0014 경계 의미론 파리티 확인.
- **체크포인트 오귀속 해소** (1일차 `awaker-82d56acf…` 패턴 재현): 구 동작이면
  shown이 마지막 Started인 YouTube 파일로 갔을 상황에서 **shown이 발동 세션
  (sbrowser Resume) 파일에 ordinal 0으로 기록** — 재겨냥 정상. DB
  `checkpoint_events` row도 같은 세션·ordinal 0·leftWithinMinute=1로 정합.
  시트는 전환 2.5s 후 스크린샷엔 잔존(폴 주기 ≤5s 이내라 판정 불가, 이후
  Dismiss는 choice 라인 부재·DB choice NULL로 간접 확인).
- **도구 회귀**: 1일차 일치 건 `awaker-3e2e888c…` 재대조 — 동일 수치 일치
  (enter Δ0ms, exit Δ−2229ms, 채점 76.7%).
- **관찰**: ① N1 귀속 규칙의 실기기 발현 — 파일 `n1` 라인은 이탈 시점 aim
  세션(Chrome) 파일에, DB `setN1`은 shown 이벤트 row(sbrowser)에 기록됐다.
  finding 수용 코멘트의 결정(choice→`shownSession`, N1→`aim.current`)대로의
  동작이고 N1 지표 원본인 DB가 shown 세션을 가리키므로 실해 없음 — 다만 파일
  라인과 DB가 서로 다른 세션을 가리키는 비대칭이 실측으로 확인됐음을 기록.
  ② adb 스와이프는 IMU 시그니처가 없어(폰 정지) 휴리스틱 채점은 이 시나리오
  에서 무의미(미탐 대량) — 파리티 기준 아님. ③ choice 경합(비차단 표면화
  항목)은 이번에 탭 없음이라 미관찰 — 어제 구 APK choice 2건은 DB·로그 정합
  (이슈 06 코멘트). 로그: `day2/parity`·`day2/parity-b`
  (`~/.claude/jobs/f803f8ba/tmp/logs/`).
