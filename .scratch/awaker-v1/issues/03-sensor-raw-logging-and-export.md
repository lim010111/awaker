...
- [x] 세션 중에만 기록되고 세션 밖에서는 샘플링이 정지한다
- [x] export한 로그를 노트북에서 파싱해 세션 하나의 센서 타임라인을 플롯할 수 있다
- [x] 스키마에 버전 필드와 EMA 프로브 이벤트 타입이 예약돼 있다
- [x] 샘플링 활성 세션의 배터리 소모 실측치(시간당 %)가 이슈 코멘트에 기록된다
...
**2026-07-19 (agent)**: 실기기 1일차 — 실사용 로그 31파일(~64MB, 세션 커버 6.0h) pull:

- 센서 라인이 세션 경계 안에서만 존재(전수 스캔, 세션 밖 샘플 0) — AC1 체크.
- Chrome 세션(aa14f672)을 `cli plot`으로 노트북에서 플롯 생성(gyro/accel/light
  + scroll 타임라인) — AC2 체크.
- 배터리: 일일 11.4mAh(이슈 02 코멘트)에 활성 샘플링 ~6.0h 포함 → 활성 시간당
  ≈1.9mAh, 용량(4,800mAh)의 **0.04%/h** 상한 — AC4 체크.
- 관측 샘플레이트 gyro/accel ~48Hz — 명세 대역(50–100Hz) 하한 부근, replay 파싱 정상.
