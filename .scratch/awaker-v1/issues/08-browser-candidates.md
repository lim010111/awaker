...
- [x] 실기기: 본인 브라우저(Samsung Internet/Chrome 등)에서 세션 시작 + 센서/scroll 라인이 로그에 기록된다
...
**2026-07-19 (agent)**: 실기기 1일차 — 브라우저 실사용 실증: 삼성브라우저beta
세션 9(scroll 1,750), Chrome 세션 6(scroll 690), 센서 라인 동반 — AC3 체크.
teacher 룰 전이도 브라우저에서 발화(sbrowser enter 9, chrome 2), 체크포인트
16회 노출 — 유저 1일 체감 "인터넷 사용할 때는 바로바로 잘 떴다". AC4는 남은
절반이 핵심: 그 발화들이 무지성 스크롤 중이었는지 정독 중이었는지(정독 오탐
여부) 유저 체감 확인 후 판단. 동적 탐지의 가치 실증: 이 기기 브라우저 셋은
Chrome + **Samsung Internet Beta** + AdBlock Browser — 하드코딩이었다면 베타
패키지(`sbrowser.beta`)를 놓쳤을 것.
